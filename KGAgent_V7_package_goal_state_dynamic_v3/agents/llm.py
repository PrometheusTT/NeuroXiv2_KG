# Enhanced version of llm.py with improved reasoning guidance
...
# Enhanced prompt with stronger emphasis on context retention and query construction
STEP_SYSTEM_PROMPT = """
...
Before generating a new query, ALWAYS check the Workspace State. If the information you need (e.g., a target region name) is 
already available in the workspace, USE IT directly in your next action's arguments instead of re-querying for it.

QUERY CONSTRUCTION RULES:
1. NEVER return entire nodes (e.g., RETURN n) - this causes errors. Always specify properties: RETURN n.name, n.id
2. Use proper aggregation with COUNT: If using COUNT, include GROUP BY clause for other fields
3. When filtering, try CONTAINS instead of exact equality for text matching
4. Triple-check syntax for complex queries with multiple MATCH clauses
...
- IMPORTANT: Once you identify the top region, SAVE IT in workspace with add_important_result
...
CRITICAL WORKFLOW:
1. First determine which regions have Car3 subclass
2. When you find the region with highest Car3 proportion, SAVE IT with:
   add_important_result("top_car3_region", region_name, "Region with highest Car3 proportion")
3. For subsequent queries, USE this saved region name directly from workspace
...
1. If you haven't found Car3 data yet, try queries like:
   - MATCH (r:Region)-[:HAS_SUBCLASS]->(s:Subclass) WHERE s.name CONTAINS 'Car3' RETURN r.name, s.name, s.proportion
   - MATCH (s:Subclass) WHERE s.name CONTAINS 'Car3' RETURN s.name, s.proportion
   - MATCH (n) WHERE ANY(prop IN keys(n) WHERE toString(n[prop]) CONTAINS 'Car3') RETURN n.name, labels(n) LIMIT 10
...
""")
        ...
        if data.get("action") == "final" and self.step_count < 2:
            ...
            data = {
                ...
                "args": {
                    "query": "MATCH (s:Subclass) WHERE s.name CONTAINS 'Car3' OR ANY(prop IN keys(s) WHERE toString(s[prop]) CONTAINS 'Car3') RETURN s.name, labels(s) LIMIT 20"
                }
            }
...
4. Identify the region with highest Car3 proportion and SAVE IT with add_important_result
...
"""
        ...
        # Add query validation for execute_cypher to prevent returning entire nodes
        if data.get("action") == "execute_cypher":
            query = data.get("args", {}).get("query", "")
            improved_query = self._validate_and_improve_query(query)
            if improved_query != query:
                print("[LLM] Improving query to prevent returning entire nodes")
                data["args"]["query"] = improved_query
                # Add reasoning about query improvement
                if "reasoning_chain" in data:
                    data["reasoning_chain"].append(
                        f"Improved query to prevent returning entire nodes: {improved_query}")
...
"""}
    ...
    def _validate_and_improve_query(self, query: str) -> str:
        """
        Validate and improve a Cypher query to prevent common errors.
        """
        # Check for returning entire nodes
        return_clause_pattern = r'\bRETURN\s+([^.]+?)\b(?![.)])'

        # Find all matches of the pattern
        matches = re.finditer(return_clause_pattern, query)

        # Process the query to replace entire node returns with specific properties
        result = query
        for match in matches:
            ...
            if var_name and var_name.isalnum() and not any(x in var_name for x in ['.', '(', ')', ' ']):
                # Replace with returning the id and labels of the node
                replacement = f"RETURN {var_name}.name as {var_name}_name, labels({var_name}) as {var_name}_labels"
                result = result.replace(f"RETURN {var_name}", replacement)
    ...
    def _parse_json_robust(self, s: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "action": "execute_cypher",
            "args": {
                "query": "MATCH (n) WHERE ANY(prop IN keys(n) WHERE toString(n[prop]) CONTAINS 'Car3') RETURN n.name, labels(n) LIMIT 10"
            }
        }
            ...
            if not self._has_car3_data(history):
                ...
                if not any("Subclass" in q for q in queries_tried):
                    return {
                        ...
                        "action": "execute_cypher",
                        "args": {
                            "query": "MATCH (s:Subclass) WHERE s.name CONTAINS 'Car3' RETURN s.name, s.proportion"
                        }
                    }
                    ...
                    return {
                        ...
                        "action": "execute_cypher",
                        "args": {
                            "query": "MATCH (r:Region)-[:HAS_SUBCLASS]->(s:Subclass) WHERE s.name CONTAINS 'Car3' RETURN r.name as region, s.name as subclass, s.proportion"
                        }
                    }
                else:
                    return {
                        ...
                        "action": "execute_cypher",
                        "args": {
                            "query": "MATCH (n) WHERE ANY(prop IN keys(n) WHERE toString(n[prop]) CONTAINS 'Car3') RETURN n.name, labels(n) as labels LIMIT 20"
                        }
                    }
        ...
        for entry in history[-5:]:  # Last 5 actions for context
            ...
            if result.get("ok"):
                if "rows" in result:
                    ...
                    # Add data samples for small result sets
                    if result.get("rows", 0) > 0 and result.get("rows", 0) <= 5 and result.get("df") is not None:
                        try:
                            if isinstance(result["df"], list) and len(result["df"]) > 0:
                                formatted.append(f"  Data Sample: {result['df'][0]}")
                            elif isinstance(result["df"], dict):
                                formatted.append(f"  Data: {result['df']}")
                        except:
                            pass
                else:
                ...
                if "diagnostics" in result:
                    ...
                    if "suggestions" in diag and diag["suggestions"]:
                        formatted.append(f"  Fix Suggestions: {', '.join(diag['suggestions'])}")
        ...
        for entry in history:
            if entry.get("action") == "execute_cypher" and entry.get("result", {}).get("ok"):
                if entry.get("result", {}).get("rows", 0) > 0:
                    ...
                    # Try to extract actual data for small result sets
                    if entry.get("result", {}).get("rows", 0) <= 5 and "df" in entry.get("result", {}):
                        try:
                            if isinstance(entry["result"]["df"], list):
                                finding["data_sample"] = entry["result"]["df"][:5]
                            elif isinstance(entry["result"]["df"], dict):
                                finding["data_sample"] = entry["result"]["df"]
                        except:
                            pass
    ...
    def _summarize_analysis_steps(self, history: List[Dict[str, Any]]) -> str:
        ...
        for entry in history:
            if entry.get("action") == "execute_cypher":
                result = entry.get("result", {})
                if result.get("ok"):
                    # Include data snippets for important findings
                    data_snippet = ""
                    if result.get("rows", 0) > 0 and result.get("rows", 0) <= 3:
                        try:
                            if isinstance(result.get("df"), list) and result["df"]:
                                data_snippet = f" - Sample: {result['df'][0]}"
                            elif isinstance(result.get("df"), dict):
                                data_snippet = f" - Data: {result['df']}"
                        except:
                            pass

                    steps.append(
                        f"- {entry.get('thought', 'Query executed')}: {result.get('rows', 0)} rows found{data_snippet}")
                else:
                    steps.append(
                        f"- {entry.get('thought', 'Query attempted')}: Failed - {result.get('error', 'Unknown error')}")
