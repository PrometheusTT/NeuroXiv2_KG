...
class MERFISHSubregionMapper:
    ...
    ⭐ 核心修复：从ME节点的id字段正确提取parent_id（格式："parent_id_me_id"）
    """
    ...
    def __init__(self, data_path: Path, ccf_tree_json: Path, ccf_me_json: Path):
        ...
        self.json_id_to_me_acronym = {}  # ⭐ 新增：使用完整ID映射
        ...
        try:
            ...
            for acronym, info in all_nodes.items():
                ...
                if '-ME' in acronym and parent_acronym:
                    ...
                    # ⭐ 核心修复：从node_id提取parent_id
                    # node_id格式："parent_ccf_id_me_id"（例如："943_953"）
                    if node_id and isinstance(node_id, str) and '_' in node_id:
                        try:
                            # 提取parent_ccf_id和me_id
                            parts = node_id.split('_')
                            parent_ccf_id = int(parts[0])  # 下划线前的部分
                            me_id = int(parts[1])  # 下划线后的部分

                            # 建立parent_id到ME_acronym的映射
                            self.parent_id_to_me_acronym[parent_ccf_id] = acronym

                            # ⭐ 关键：建立完整json_id到ME_acronym的映射
                            json_id = f"{parent_ccf_id}_{me_id}"
                            self.json_id_to_me_acronym[json_id] = acronym

                        except (ValueError, IndexError) as e:
                            logger.warning(f"无法解析ME节点ID: {node_id}, 错误: {e}")

                    # 如果node_id无效，尝试后备方案
                    elif parent_acronym:
                        # 后备方案1：从subregion_id映射查找
                        if parent_acronym in all_nodes:
                            parent_node_id = all_nodes[parent_acronym]['node_id']
                            if parent_node_id is not None:
                                try:
                                    parent_ccf_id = int(parent_node_id)
                                    self.parent_id_to_me_acronym[parent_ccf_id] = acronym
                                except (ValueError, TypeError):
                                    pass

                        # 后备方案2：从标准CCF树查找
                        if parent_acronym in self.acronym_to_id:
            ...
            logger.info(f"  - 建立了 {len(self.json_id_to_me_acronym)} 个JSON ID映射")

            # 显示样例映射（用于验证）
            if self.json_id_to_me_acronym:
                sample_items = list(self.json_id_to_me_acronym.items())[:5]
                logger.info(f"  样例JSON ID映射:")
                for json_id, me_acronym in sample_items:
                    logger.info(f"    {json_id} -> {me_acronym}")
    ...
    def _traverse_ccf_me_tree(self, node, parent_acronym=None, grandparent_acronym=None):
        ...
        ⭐ 这个方法现在由 load_ccf_me_tree_for_subregions 的两遍遍历替代
        但保留它以防其他地方调用
        """
        acronym = node.get('acronym', '')
        node_id = node.get('id')
    ...
    def load_me_subregion_annotation(self) -> bool:
        ...
        try:
            ...
            logger.info(f"  - 已有 {len(self.json_id_to_me_acronym)} 个JSON ID->ME映射")
    ...
    def map_cells_to_me_subregions(self, cells_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(0, len(cells_df), batch_size):
            ...
            for x, y, z in zip(batch_x, batch_y, batch_z):
                if (0 <= x < self.me_annotation.shape[0] and
                    ...
                    # ⭐ 修复：使用正确的json_id格式查找ME acronym
                    # 方法1：尝试使用完整的json_id（推荐）
                    json_id = f"{parent_id}_{voxel}"
                    me_acronym = self.json_id_to_me_acronym.get(json_id)

                    # 方法2：后备方案，直接用parent_id查找
                    if not me_acronym:
        ...
        logger.info(f"    - voxel为0: {voxel_zero} ({voxel_zero / len(cells_df) * 100:.2f}%)")
        logger.info(f"    - voxel不在pkl: {voxel_not_in_pkl}")
        logger.info(f"    - parent不在JSON: {parent_not_in_json} (非皮层区域)")
        logger.info(f"    - 超出边界: {out_of_bounds}")

        # ⭐ 新增：计算在有ME注释的细胞中的成功率
        cells_with_me_annotation = len(cells_df) - voxel_zero - out_of_bounds
        if cells_with_me_annotation > 0:
            success_rate_in_annotated = mapped_count / cells_with_me_annotation * 100
            logger.info(f"  ⓘ 在有ME注释的细胞中:")
            logger.info(f"    - 成功映射: {mapped_count}/{cells_with_me_annotation} ({success_rate_in_annotated:.2f}%)")
...
def verify_hierarchical_projections(neo4j_connector, database='neo4j'):
    """
    验证所有投射关系
    """
    logger.info("=" * 80)
    logger.info("验证知识图谱中的投射关系")
    logger.info("=" * 80)

    with neo4j_connector.driver.session(database=database) as session:
        # 1. Neuron -> Region
        query1 = """
        MATCH (n:Neuron)-[r:PROJECT_TO]->(reg:Region)
        RETURN count(r) as count, 
               avg(r.projection_length) as avg_length,
               max(r.projection_length) as max_length
        """
        result1 = session.run(query1)
        record1 = result1.single()
        logger.info(f"\n✓ Neuron -> Region:")
        logger.info(f"   数量: {record1['count']}")
        logger.info(f"   平均长度: {record1['avg_length']:.2f}")
        logger.info(f"   最大长度: {record1['max_length']:.2f}")

        # 2. Neuron -> Subregion
        query2 = """
        MATCH (n:Neuron)-[r:PROJECT_TO]->(sr:Subregion)
        WHERE r.target_level = 'subregion' AND r.source_level IS NULL
        RETURN count(r) as count,
               avg(r.projection_length) as avg_length
        """
        ...
        logger.info(f"\n✓ Neuron -> Subregion:")
        logger.info(f"   数量: {record2['count']}")
        logger.info(f"   平均长度: {record2['avg_length']:.2f}")

        # 3. Region -> Subregion
        query3 = """
        MATCH (r:Region)-[p:PROJECT_TO]->(sr:Subregion)
        WHERE p.aggregation_level = 'region_to_subregion'
        RETURN count(p) as count,
               avg(p.projection_length) as avg_length
        """
        result3 = session.run(query3)
        record3 = result3.single()
        logger.info(f"\n✓ Region -> Subregion:")
        logger.info(f"   数量: {record3['count']}")
        logger.info(f"   平均长度: {record3['avg_length']:.2f}")

        # 4. Subregion -> Subregion
        query4 = """
        MATCH (s1:Subregion)-[p:PROJECT_TO]->(s2:Subregion)
        WHERE p.aggregation_level = 'subregion_to_subregion'
        RETURN count(p) as count,
               avg(p.projection_length) as avg_length
        """
        ...
        logger.info(f"\n✓ Subregion -> Subregion:")
        logger.info(f"   数量: {record4['count']}")
        logger.info(f"   平均长度: {record4['avg_length']:.2f}")

        # 5. 显示样例
        logger.info(f"\n样例投射关系:")
        query5 = """
        MATCH (s1)-[p:PROJECT_TO]->(s2)
        WHERE p.projection_length > 0
        RETURN labels(s1)[0] as source_type, 
               s1.acronym as source, 
               labels(s2)[0] as target_type,
               s2.acronym as target,
               p.projection_length as length,
               p.aggregation_level as agg_level
        ORDER BY p.projection_length DESC
        LIMIT 10
        """
        result5 = session.run(query5)
        for record in result5:
            source_label = record['source_type']
            target_label = record['target_type']
            logger.info(f"   {source_label}:{record['source']} -> {target_label}:{record['target']} "
                        f"(长度: {record['length']:.2f}, 级别: {record['agg_level']})")
...
def main(data_dir: str = "../data",
    ...
    try:
        ...
        verify_hierarchical_projections(neo4j_conn, database_name)
