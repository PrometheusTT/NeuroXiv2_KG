...
class BrainRegionFingerprints:
    ...
    def compute_morphology_signature(self, region: str) -> np.ndarray:
        ...
        WITH COLLECT(DISTINCT n1) + COLLECT(DISTINCT n2) + COLLECT(DISTINCT n3) AS ns
        UNWIND ns AS n
        WITH DISTINCT n
        WHERE n IS NOT NULL
        RETURN
          ...
          n.axonal_maximum_branch_order AS axonal_max_branch_order,
          ...
          n.dendritic_maximum_branch_order AS dendritic_max_branch_order
        """
        ...
        signature = np.array([df[feat].mean() for feat in features])
        return signature
    ...
    def compute_projection_signature(self, region: str) -> np.ndarray:
        ...
        WHERE p.weight IS NOT NULL AND p.projection_length > 0

        WITH t.acronym AS tgt_subregion,
             SUM(p.weight) AS total_len_to_tgt
        RETURN
          tgt_subregion,
          total_len_to_tgt
        ORDER BY total_len_to_tgt DESC
        """

        with self.driver.session() as session:
            result = session.run(query, region=region)
            data = {record['tgt_subregion']: record['total_len_to_tgt']
                    for record in result}
