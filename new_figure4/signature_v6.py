"""
脑区指纹计算与可视化 - v6 PCA版本 (64维形态特征)
根据知识图谱计算分子指纹、形态指纹和投射指纹，并分析区域间的mismatch

向量标准化流程（与clustering_exploration_v4保持一致）：
【形态向量】
  原始维度: 64 (axon 32 + dendrite 32)
  Step 1: Log1p变换
  Step 2: Z-score标准化（按列）
  Step 3: PCA (方差解释保持在 95%左右)
【分子向量】
  原始维度: ~338 subclasses
  Step 1: 剪枝 -> 去掉零值subclass
  Step 2: Z-score标准化（按列）
  Step 3: PCA (方差解释保持在 95%左右)
【投射向量】
  原始维度: ~365 脑区
  Step 1: 剪枝 -> 去掉零值脑区
  Step 2: Log1p变换（值域跨度大）
  Step 3: Z-score标准化（按列）
  Step 4: PCA (方差解释保持在 95%左右)
