"""
任务1：Neuron-level 多模态预测实验 (V4 科学严谨版)
===============================================
测试多模态信息（形态+局部分子环境）是否能提高神经元投射预测

关键改进 (V4):
1. Gene特征：CLR变换后再StandardScaler（compositional数据正确处理）
2. 主指标：Cosine Similarity + JSD Similarity（分布预测语义）
3. 可视化重构：
   - 主图：Δ performance分布图（一眼秒懂）
   - 补充：Paired dot plot展示逐样本提升
4. R²放入补充材料
...
class NeuronMultimodalPredictorV4:
    """神经元多模态投射预测器 V4（科学严谨版）"""
    ...
    def prepare_features_axon_only(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, List[str]]:
        ...
        关键改进：
        - Morph: log1p + RobustScaler（处理长尾分布）
        - Gene: CLR + StandardScaler（正确处理compositional数据）
        """
        print("\n准备Axon-only特征矩阵...")
        ...
        # log1p处理长尾分布，RobustScaler对outlier鲁棒
    ...
    def prepare_features_full_morph(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, List[str]]:
        """
        准备full-morph数据集的特征（只用有dendrite的神经元）
    ...
    def train_and_predict(self, X: np.ndarray, Y: np.ndarray,
        ...
        jsd_sims = np.zeros(n_samples)  # 新增
    ...
    def statistical_analysis(self) -> Dict:
        ...
        for dataset in ['full', 'axon']:
            ...
            # JSD similarity
            jsd_morph = self.results[f'{prefix}morph_only'].jsd_similarities
            jsd_gene = self.results[f'{prefix}gene_only'].jsd_similarities
            jsd_multi = self.results[f'{prefix}morph_gene'].jsd_similarities
            jsd_shuffle = self.results[f'{prefix}shuffle'].jsd_similarities
            ...
            comparisons_jsd = [
                ('Morph+Gene vs Morph-only', jsd_multi, jsd_morph),
                ('Morph+Gene vs Gene-only', jsd_multi, jsd_gene),
                ('Morph+Gene vs Shuffle', jsd_multi, jsd_shuffle),
            ]

            for name, sim1, sim2 in comparisons_jsd:
                stat, p_val = wilcoxon(sim1, sim2, alternative='greater')
                diff = sim1 - sim2
                cohens_d = np.mean(diff) / np.std(diff) if np.std(diff) > 0 else 0
                improve = (np.mean(sim1) - np.mean(sim2)) / np.mean(sim2) * 100 if np.mean(sim2) > 0 else 0

                stats[f'{dataset}_jsd_{name}'] = {
                    'wilcoxon_stat': stat, 'p_value': p_val,
                    'cohens_d': cohens_d, 'improvement_pct': improve
                }
    ...
    def visualize_results(self, output_dir: str = "."):
        ...
        self._plot_delta_distribution(output_dir)  # 主图1：Δ分布
        self._plot_paired_improvement(output_dir)   # 主图2：Paired dots
        self._plot_comparison_summary(output_dir)   # 补充：汇总对比
        self._plot_target_heatmap(output_dir)       # 补充：Target heatmap
    ...
    def _plot_delta_distribution(self, output_dir: str):
        ...
        一眼秒懂的核心图：展示multi相对于各baseline的提升分布
        """
    ...
    def _plot_comparison_summary(self, output_dir: str):
        ...
        for ax_idx, (dataset, title) in enumerate([('full', 'Full-morph'), ('axon', 'Axon-only')]):
            ax = axes[ax_idx]
            prefix = f'{dataset}_'
    ...
    def _plot_target_heatmap(self, output_dir: str):
        ...
        for i in range(len(sorted_targets)):
            if data[i, 2] == data[i].max():  # Morph+Gene is best
    ...
    def run_full_pipeline(self, output_dir: str = "./task1_results", model_name: str = "Ridge"):
        """运行完整流程"""
        print("\n" + "=" * 80)
        print("任务1: Neuron-level 多模态投射预测 (V4 科学严谨版)")
        print("=" * 80)
...
=== Cosine Similarity ===
  Morph-only: {self.results['full_morph_only'].mean_cosine_sim:.4f} ± {self.results['full_morph_only'].std_cosine_sim:.4f}
  Gene-only:  {self.results['full_gene_only'].mean_cosine_sim:.4f} ± {self.results['full_gene_only'].std_cosine_sim:.4f}
  Morph+Gene: {self.results['full_morph_gene'].mean_cosine_sim:.4f} ± {self.results['full_morph_gene'].std_cosine_sim:.4f}
  Shuffle:    {self.results['full_shuffle'].mean_cosine_sim:.4f} ± {self.results['full_shuffle'].std_cosine_sim:.4f}

=== JSD Similarity (分布预测指标) ===
  Morph-only: {self.results['full_morph_only'].mean_jsd_sim:.4f} ± {self.results['full_morph_only'].std_jsd_sim:.4f}
  Gene-only:  {self.results['full_gene_only'].mean_jsd_sim:.4f} ± {self.results['full_gene_only'].std_jsd_sim:.4f}
  Morph+Gene: {self.results['full_morph_gene'].mean_jsd_sim:.4f} ± {self.results['full_morph_gene'].std_jsd_sim:.4f}
  Shuffle:    {self.results['full_shuffle'].mean_jsd_sim:.4f} ± {self.results['full_shuffle'].std_jsd_sim:.4f}

=== 统计检验 (Cosine) ===
  Morph+Gene vs Morph-only: p={stats['full_cos_Morph+Gene vs Morph-only']['p_value']:.2e}, Cohen's d={stats['full_cos_Morph+Gene vs Morph-only']['cohens_d']:.3f}
  Morph+Gene vs Gene-only:  p={stats['full_cos_Morph+Gene vs Gene-only']['p_value']:.2e}, Cohen's d={stats['full_cos_Morph+Gene vs Gene-only']['cohens_d']:.3f}
  Morph+Gene vs Shuffle:    p={stats['full_cos_Morph+Gene vs Shuffle']['p_value']:.2e}, Cohen's d={stats['full_cos_Morph+Gene vs Shuffle']['cohens_d']:.3f}

=== 数据处理改进 ===
  - Gene: CLR变换 + StandardScaler（正确处理compositional数据）
  - Morph: log1p + RobustScaler（处理长尾分布和outliers）
""")
...
def main():
    ...
    OUTPUT_DIR = "./task1_neuron_multimodal_results_v4"
    MODEL_NAME = "Ridge"

    with NeuronMultimodalPredictorV4(NEO4J_URI, NEO4J_USER, NEO4J_PASSWORD, NEO4J_DATABASE) as predictor:
