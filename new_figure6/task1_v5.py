...
class NeuronMultimodalPredictorV5:
    ...
    def _plot_delta_distribution(self, output_dir: str):
        ...
        # 绘制KDE
        sns.kdeplot(delta_multi_morph, ax=ax, color='#3498DB', linewidth=2.5,
                    label=f'Multi − Morph (med={np.median(delta_multi_morph):.3f})')
        sns.kdeplot(delta_multi_gene, ax=ax, color='#27AE60', linewidth=2.5,
                    label=f'Multi − Gene (med={np.median(delta_multi_gene):.3f})')
        sns.kdeplot(delta_multi_shuffle, ax=ax, color='#95A5A6', linewidth=2.5,
        ...
        ax.axvline(x=np.median(delta_multi_morph), color='#3498DB', linestyle=':', linewidth=2, alpha=0.8)
        ax.axvline(x=np.median(delta_multi_gene), color='#27AE60', linestyle=':', linewidth=2, alpha=0.8)

        ax.set_xlabel('Δ Cosine Similarity', fontsize=12, fontweight='medium')
        ax.set_ylabel('Density', fontsize=12, fontweight='medium')
        ax.set_title('Performance Gain Distribution (Cosine Similarity)', fontsize=13, fontweight='bold', pad=15)

        # 图例放在不遮挡数据的位置
        ax.legend(fontsize=10, loc='upper left', framealpha=0.95, edgecolor='gray', fancybox=True)
        ...
        stats_text = f'vs Morph: p={p_morph:.2e}\nvs Gene: p={p_gene:.2e}'
        ax.text(0.98, 0.98, stats_text, transform=ax.transAxes, fontsize=10,
                verticalalignment='top', horizontalalignment='right',
                bbox=dict(boxstyle='round,pad=0.4', facecolor='#FFF9E6', alpha=0.9, edgecolor='#E0C060'))

        for spine in ['top', 'right']:
            ax.spines[spine].set_visible(False)

        fig_a.tight_layout()
        self._save_figure(fig_a, output_dir, "1a_delta_cosine_distribution.png")
        ...
        sns.kdeplot(delta_multi_morph_jsd, ax=ax, color='#3498DB', linewidth=2.5,
                    label=f'Multi − Morph (med={np.median(delta_multi_morph_jsd):.3f})')
        sns.kdeplot(delta_multi_gene_jsd, ax=ax, color='#27AE60', linewidth=2.5,
                    label=f'Multi − Gene (med={np.median(delta_multi_gene_jsd):.3f})')
        sns.kdeplot(delta_multi_shuffle_jsd, ax=ax, color='#95A5A6', linewidth=2.5,
                    label=f'Multi − Shuffle (med={np.median(delta_multi_shuffle_jsd):.3f})')

        ax.axvline(x=0, color='black', linestyle='--', linewidth=1.5, alpha=0.7, zorder=1)
        ax.axvline(x=np.median(delta_multi_morph_jsd), color='#3498DB', linestyle=':', linewidth=2, alpha=0.8)
        ax.axvline(x=np.median(delta_multi_gene_jsd), color='#27AE60', linestyle=':', linewidth=2, alpha=0.8)

        ax.set_xlabel('Δ JSD Similarity', fontsize=12, fontweight='medium')
        ax.set_ylabel('Density', fontsize=12, fontweight='medium')
        ax.set_title('Performance Gain Distribution (JSD Similarity)', fontsize=13, fontweight='bold', pad=15)
        ax.legend(fontsize=10, loc='upper left', framealpha=0.95, edgecolor='gray', fancybox=True)
        ax.grid(axis='y', alpha=0.3, linestyle='-', linewidth=0.5)
        ax.set_axisbelow(True)
        ...
        stats_text = f'vs Morph: p={p_morph_jsd:.2e}\nvs Gene: p={p_gene_jsd:.2e}'
        ax.text(0.98, 0.98, stats_text, transform=ax.transAxes, fontsize=10,
                verticalalignment='top', horizontalalignment='right',
                bbox=dict(boxstyle='round,pad=0.4', facecolor='#FFF9E6', alpha=0.9, edgecolor='#E0C060'))
        ...
        fig_b.tight_layout()
        self._save_figure(fig_b, output_dir, "1b_delta_jsd_distribution.png")
        ...
        ax.axvline(x=np.median(delta_multi_morph), color='#3498DB', linestyle=':', linewidth=2)
        ax.axvline(x=np.median(delta_multi_gene), color='#27AE60', linestyle=':', linewidth=2)
        ax.set_xlabel('Δ Cosine Similarity', fontsize=11, fontweight='medium')
        ax.set_ylabel('Density', fontsize=11, fontweight='medium')
        ax.set_title('A.Cosine Similarity Gain', fontsize=12, fontweight='bold', pad=10)
        ax.legend(fontsize=9, loc='upper left', framealpha=0.95)
        ax.grid(axis='y', alpha=0.3)
        ax.text(0.98, 0.98, f'vs Morph: p={p_morph:.2e}\nvs Gene: p={p_gene:.2e}',
        ...
        sns.kdeplot(delta_multi_shuffle_jsd, ax=ax, color='#95A5A6', linewidth=2.5,
                    label=f'Multi − Shuffle (med={np.median(delta_multi_shuffle_jsd):.3f})')
        ax.axvline(x=0, color='black', linestyle='--', linewidth=1.5, alpha=0.7)
        ax.axvline(x=np.median(delta_multi_morph_jsd), color='#3498DB', linestyle=':', linewidth=2)
        ax.axvline(x=np.median(delta_multi_gene_jsd), color='#27AE60', linestyle=':', linewidth=2)
        ax.set_xlabel('Δ JSD Similarity', fontsize=11, fontweight='medium')
        ax.set_ylabel('Density', fontsize=11, fontweight='medium')
        ax.set_title('B.JSD Similarity Gain', fontsize=12, fontweight='bold', pad=10)
        ax.legend(fontsize=9, loc='upper left', framealpha=0.95)
        ax.grid(axis='y', alpha=0.3)
        ax.text(0.98, 0.98, f'vs Morph: p={p_morph_jsd:.2e}\nvs Gene: p={p_gene_jsd:.2e}',
                transform=ax.transAxes, fontsize=9, verticalalignment='top', horizontalalignment='right',
                bbox=dict(boxstyle='round', facecolor='#FFF9E6', alpha=0.9))
        for spine in ['top', 'right']:
            ax.spines[spine].set_visible(False)
    ...
    def _plot_paired_improvement(self, output_dir: str):
        ...
        fig_a, ax = plt.subplots(figsize=(7, 7))

        # 绘制连接线
        for i in range(n_show):
            color = '#27AE60' if sim_multi[i] > sim_morph[i] else '#E74C3C'
            alpha = 0.4 if sim_multi[i] > sim_morph[i] else 0.25
            ax.plot([0, 1], [sim_morph[i], sim_multi[i]], color=color, alpha=alpha, linewidth=0.8)

        # 绘制散点
        ax.scatter(np.zeros(n_show), sim_morph, s=25, alpha=0.6, c='#3498DB',
                   edgecolors='white', linewidth=0.5, zorder=5, label='Morph-only')
        ax.scatter(np.ones(n_show), sim_multi, s=25, alpha=0.6, c='#E74C3C',
                   edgecolors='white', linewidth=0.5, zorder=5, label='Morph+Gene')

        # 均值线
        ax.plot([0, 1], [np.mean(sim_morph), np.mean(sim_multi)], 'k-', linewidth=3.5, zorder = 10,
        label = f'Mean:  {np.mean(sim_morph):.3f} → {np.mean(sim_multi):.3f}')
        ax.scatter([0, 1], [np.mean(sim_morph), np.mean(sim_multi)], s=100, c='black',
                   edgecolors='white', linewidth=2, zorder=11)

        ax.set_xticks([0, 1])
        ax.set_xticklabels(['Morph-only', 'Morph+Gene'], fontsize=12, fontweight='medium')
        ax.set_ylabel('Cosine Similarity', fontsize=12, fontweight='medium')
        ax.set_title('Per-Neuron Improvement:  Morph-only → Morph+Gene\n(Green=Improved, Red=Decreased)',
        ...
        ax.text(0.5, 0.02, f'{n_improved}/{n_show} ({n_improved / n_show:.1%}) improved',
                ...
                bbox=dict(boxstyle='round,pad=0.4', facecolor='#E8F8E8', alpha=0.95, edgecolor='#27AE60'))

        for spine in ['top', 'right']:
            ax.spines[spine].set_visible(False)

        fig_a.tight_layout()
        self._save_figure(fig_a, output_dir, "2a_paired_morph_vs_multi.png")

        # ===================== Panel B: Multi vs Gene-only =====================
        fig_b, ax = plt.subplots(figsize=(7, 7))

        for i in range(n_show):
            color = '#27AE60' if sim_multi[i] > sim_gene[i] else '#E74C3C'
            alpha = 0.4 if sim_multi[i] > sim_gene[i] else 0.25
            ax.plot([0, 1], [sim_gene[i], sim_multi[i]], color=color, alpha=alpha, linewidth=0.8)

        ax.scatter(np.zeros(n_show), sim_gene, s=25, alpha=0.6, c='#27AE60',
                   edgecolors='white', linewidth=0.5, zorder=5, label='Gene-only')
        ax.scatter(np.ones(n_show), sim_multi, s=25, alpha=0.6, c='#E74C3C',
                   edgecolors='white', linewidth=0.5, zorder=5, label='Morph+Gene')

        ax.plot([0, 1], [np.mean(sim_gene), np.mean(sim_multi)], 'k-', linewidth=3.5, zorder=10,
                label=f'Mean: {np.mean(sim_gene):.3f} → {np.mean(sim_multi):.3f}')
        ax.scatter([0, 1], [np.mean(sim_gene), np.mean(sim_multi)], s=100, c='black',
                   edgecolors='white', linewidth=2, zorder=11)

        ax.set_xticks([0, 1])
        ax.set_xticklabels(['Gene-only', 'Morph+Gene'], fontsize=12, fontweight='medium')
        ax.set_ylabel('Cosine Similarity', fontsize=12, fontweight='medium')
        ax.set_title('Per-Neuron Improvement: Gene-only → Morph+Gene\n(Green=Improved, Red=Decreased)',
                     fontsize=12, fontweight='bold', pad=15)
        ax.set_xlim(-0.3, 1.3)
        ax.legend(fontsize=10, loc='upper center', bbox_to_anchor=(0.5, -0.08),
                  ncol=3, framealpha=0.95, edgecolor='gray')
        ...
        fig_b.tight_layout()
        self._save_figure(fig_b, output_dir, "2b_paired_gene_vs_multi.png")
        ...
        for i in range(n_show):
            color = '#27AE60' if sim_multi[i] > sim_morph[i] else '#E74C3C'
            ax.plot([0, 1], [sim_morph[i], sim_multi[i]], color=color, alpha=0.3, linewidth=0.6)
        ax.scatter(np.zeros(n_show), sim_morph, s=20, alpha=0.5, c='#3498DB',
                   edgecolors='white', linewidth=0.3, label='Morph-only')
        ax.scatter(np.ones(n_show), sim_multi, s=20, alpha=0.5, c='#E74C3C',
                   edgecolors='white', linewidth=0.3, label='Morph+Gene')
        ...
        ax.set_title('A.Morph-only → Morph+Gene', fontsize=12, fontweight='bold', pad=10)
        ...
        for i in range(n_show):
            color = '#27AE60' if sim_multi[i] > sim_gene[i] else '#E74C3C'
            ax.plot([0, 1], [sim_gene[i], sim_multi[i]], color=color, alpha=0.3, linewidth=0.6)
        ax.scatter(np.zeros(n_show), sim_gene, s=20, alpha=0.5, c='#27AE60',
                   edgecolors='white', linewidth=0.3, label='Gene-only')
        ax.scatter(np.ones(n_show), sim_multi, s=20, alpha=0.5, c='#E74C3C',
                   edgecolors='white', linewidth=0.3, label='Morph+Gene')
        ...
        ax.set_ylabel('Cosine Similarity', fontsize=11)
        ax.set_title('B.Gene-only → Morph+Gene', fontsize=12, fontweight='bold', pad=10)
        ax.legend(fontsize=9, loc='lower right', framealpha=0.95)
        ax.grid(axis='y', alpha=0.3)
        n_improved = np.sum(sim_multi > sim_gene)
        ax.text(0.5, 0.02, f'{n_improved}/{n_show} ({n_improved/n_show:.1%}) improved',
        ...
        plt.suptitle('Per-Neuron Performance Change (Green=↑, Red=↓)',
                     fontsize=13, fontweight='bold', y=1.02)
        plt.tight_layout()
        self._save_figure(fig, output_dir, "2_paired_improvement.png")

    def _plot_comparison_summary(self, output_dir:  str):
        """补充图：三个数据集各条件性能汇总 - 每个子图单独保存"""
        ...
        for prefix, title, accent_color in datasets:
            ...
            best_jsd_idx = np.argmax(jsd_means)
