...
class RegionMultimodalAnalysisV10:
    ...
    def visualize_results(self, output_dir: str = "."):
        ...
        self._plot_multimodal_vs_single_scatter(output_dir)  # 新增
    ...
    def _plot_multimodal_vs_single_scatter(self, output_dir: str):
        """
        新增核心图：多模态 vs 各单模态的散点图对比
        突出显示：
        1.对角线偏离程度（多模态带来的改变）
        2.高shift区域（单模态近但多模态远的点）
        3.同系统/跨系统的区分
        """
        from scipy.stats import gaussian_kde

        modalities = ['mol', 'morph', 'proj']
        mod_names = ['Molecular', 'Morphology', 'Projection']
        colors = ['#3498DB', '#27AE60', '#9B59B6']

        # ===================== 单独保存每个对比图 =====================
        for mod, mod_name, color in zip(modalities, mod_names, colors):
            fig, ax = plt.subplots(figsize=(9, 8))

            rank_single = self.df_pairs[f'rank_{mod}']
            rank_multi = self.df_pairs['rank_multi']
            same_system = self.df_pairs['same_system']

            # 计算关键指标
            close_mask = rank_single <= 0.05  # 单模态Top 5%
            shifted_mask = close_mask & (rank_multi > 0.30)  # 发生显著shift的
            high_shift_mask = close_mask & (rank_multi > 0.50)  # 极端shift

            # 计算偏离度
            deviation = rank_multi - rank_single

            # === 分层绘制 ===

            # Layer 1: 背景点（普通pairs，灰色）
            bg_mask = ~close_mask & ~shifted_mask
            ax.scatter(rank_single[bg_mask], rank_multi[bg_mask],
                       s=8, c='#CCCCCC', alpha=0.3, rasterized=True, zorder=1)

            # Layer 2: 同系统的close pairs（浅色）
            same_close = close_mask & same_system & ~shifted_mask
            ax.scatter(rank_single[same_close], rank_multi[same_close],
                       s=40, c=color, alpha=0.4, edgecolors='white', linewidth=0.5,
                       label=f'Close (same system)', zorder=2)

            # Layer 3: 跨系统的close pairs（中等色）
            cross_close = close_mask & ~same_system & ~shifted_mask
            ax.scatter(rank_single[cross_close], rank_multi[cross_close],
                       s=50, c=color, alpha=0.6, edgecolors='white', linewidth=0.8,
                       marker='s', label=f'Close (cross system)', zorder=3)

            # Layer 4: 发生shift的点（突出显示，红色边框）
            shifted_same = shifted_mask & same_system & ~high_shift_mask
            ax.scatter(rank_single[shifted_same], rank_multi[shifted_same],
                       s=80, c='#F39C12', alpha=0.9, edgecolors='#E74C3C', linewidth=2,
                       label=f'Shifted >30% (same sys)', zorder=4)

            shifted_cross = shifted_mask & ~same_system & ~high_shift_mask
            ax.scatter(rank_single[shifted_cross], rank_multi[shifted_cross],
                       s=90, c='#F39C12', alpha=0.9, edgecolors='#E74C3C', linewidth=2,
                       marker='s', label=f'Shifted >30% (cross sys)', zorder=4)

            # Layer 5: 极端shift的点（最突出，大红色）
            ax.scatter(rank_single[high_shift_mask], rank_multi[high_shift_mask],
                       s=120, c='#E74C3C', alpha=1.0, edgecolors='black', linewidth=2.5,
            marker = '*', label = f'Shifted >50% (n={high_shift_mask.sum()})', zorder = 5)

            # === 辅助线和区域 ===

            # 对角线
            ax.plot([0, 1], [0, 1], 'k-', linewidth=2, alpha=0.8, label='No change (y=x)')

            # 阈值线
            ax.axhline(y=0.30, color='#F39C12', linestyle='--', linewidth=2, alpha=0.8)
            ax.axhline(y=0.50, color='#E74C3C', linestyle='--', linewidth=2, alpha=0.8)
            ax.axvline(x=0.05, color='gray', linestyle=':', linewidth=2, alpha=0.6)

            # 高亮close区域
            ax.fill_betweenx([0, 1], 0, 0.05, alpha=0.08, color=color)
            ax.text(0.025, 0.02, 'Close\nPairs', ha='center', va='bottom', fontsize=9,
                    color='gray', style='italic', fontweight='bold')

            # 高亮shift区域
            ax.fill_between([0, 0.05], 0.30, 1.0, alpha=0.1, color='#E74C3C')
            ax.text(0.025, 0.65, 'Shift\nZone', ha='center', va='center', fontsize=10,
            ...
            if shifted_mask.sum() > 0:
                # 选择shift最大的几个点
                shift_values = rank_multi[shifted_mask] - rank_single[shifted_mask]
                top_shift_idx = shift_values.nlargest(3).index

                for idx in top_shift_idx[: 2]:  # 只画2个箭头避免拥挤
                    x_start = rank_single[idx]
                    y_end = rank_multi[idx]
                    y_start = x_start  # 对角线上的点

                    ax.annotate('', xy=(x_start, y_end), xytext=(x_start, y_start + 0.02),
            ...
            n_shifted_30 = shifted_mask.sum()
            n_shifted_50 = high_shift_mask.sum()
            frac_30 = n_shifted_30 / max(n_close, 1) * 100
            frac_50 = n_shifted_50 / max(n_close, 1) * 100

            stats_text = (f'Close pairs (≤5%): {n_close}\n'
                          f'Shifted to >30%: {n_shifted_30} ({frac_30:.1f}%)\n'
                          f'Shifted to >50%:  {n_shifted_50} ({frac_50:.1f}%)')
            ...
            # === 坐标轴设置 ===
            ax.set_xlabel(f'{mod_name} Distance Rank (Single Modality)', fontsize=13, fontweight='medium')
            ax.set_ylabel('Multimodal Distance Rank', fontsize=13, fontweight='medium')
            ax.set_title(f'Multimodal vs {mod_name}\n'
                         f'How multimodal integration changes pairwise relationships',
            ...
            fig.tight_layout()
            self._save_figure(fig, output_dir, f"6_multimodal_vs_{mod}.png")

        # ===================== 合并图（1x3布局） =====================
        fig, axes = plt.subplots(1, 3, figsize=(20, 7))

        for ax, mod, mod_name, color in zip(axes, modalities, mod_names, colors):
            rank_single = self.df_pairs[f'rank_{mod}']
            rank_multi = self.df_pairs['rank_multi']
            same_system = self.df_pairs['same_system']

            close_mask = rank_single <= 0.05
            shifted_mask = close_mask & (rank_multi > 0.30)
            high_shift_mask = close_mask & (rank_multi > 0.50)

            # 背景点
            bg_mask = ~close_mask
            ax.scatter(rank_single[bg_mask], rank_multi[bg_mask],
                       s=5, c='#CCCCCC', alpha=0.25, rasterized=True, zorder=1)

            # Close但没shift的点
            close_no_shift = close_mask & ~shifted_mask
            ax.scatter(rank_single[close_no_shift], rank_multi[close_no_shift],
                       s=30, c=color, alpha=0.5, edgecolors='white', linewidth=0.5, zorder=2)

            # Shifted的点
            ax.scatter(rank_single[shifted_mask & ~high_shift_mask],
                       rank_multi[shifted_mask & ~high_shift_mask],
                       s=60, c='#F39C12', alpha=0.9, edgecolors='#E74C3C', linewidth=1.5,
                       label=f'>30%', zorder=3)

            # 极端shift
            ax.scatter(rank_single[high_shift_mask], rank_multi[high_shift_mask],
                       s=100, c='#E74C3C', alpha=1.0, edgecolors='black', linewidth=2,
                       marker='*', label=f'>50%', zorder=4)

            # 辅助线
            ax.plot([0, 1], [0, 1], 'k-', linewidth=1.5, alpha=0.7)
            ax.axhline(y=0.30, color='#F39C12', linestyle='--', linewidth=1.5, alpha=0.7)
            ax.axhline(y=0.50, color='#E74C3C', linestyle='--', linewidth=1.5, alpha=0.7)
            ax.axvline(x=0.05, color='gray', linestyle=':', linewidth=1.5, alpha=0.5)

            # 高亮区域
            ax.fill_betweenx([0, 1], 0, 0.05, alpha=0.06, color=color)
            ax.fill_between([0, 0.05], 0.30, 1.0, alpha=0.08, color='#E74C3C')

            # 统计
            n_close = close_mask.sum()
            frac_30 = shifted_mask.sum() / max(n_close, 1) * 100
            frac_50 = high_shift_mask.sum() / max(n_close, 1) * 100

            ax.text(0.98, 0.02, f'>30%: {frac_30:.1f}%\n>50%: {frac_50:.1f}%',
                    transform=ax.transAxes, fontsize=10, va='bottom', ha='right',
                    bbox=dict(boxstyle='round', facecolor='white', alpha=0.9, edgecolor=color))

            ax.set_xlabel(f'{mod_name} Rank', fontsize=11)
            ax.set_ylabel('Multimodal Rank', fontsize=11)
            ax.set_title(f'vs {mod_name}', fontsize=13, fontweight='bold', pad=10)

            ax.set_xlim(-0.02, 1.02)
            ax.set_ylim(-0.02, 1.02)
            ax.set_aspect('equal')
            ax.legend(loc='upper left', fontsize=8, title='Shifted', title_fontsize=9)
            ax.grid(alpha=0.25)

            for spine in ['top', 'right']:
                ax.spines[spine].set_visible(False)

        plt.suptitle('Multimodal Integration Effect:  Rank Position Changes',
                     fontsize=15, fontweight='bold', y=1.02)
        plt.tight_layout()
        self._save_figure(fig, output_dir, "6_multimodal_vs_all_single.png")
        ...
        for ax, mod, mod_name, color in zip(axes, modalities, mod_names, colors):
            ...
            # 计算2D密度
            try:
                xy = np.vstack([rank_single, rank_multi])
                z = gaussian_kde(xy)(xy)

                # 按密度排序，低密度点先画
                idx = z.argsort()
                x, y, z = rank_single.iloc[idx], rank_multi.iloc[idx], z[idx]

                scatter = ax.scatter(x, y, c=z, s=15, cmap='viridis', alpha=0.7, rasterized=True)
                plt.colorbar(scatter, ax=ax, label='Density', shrink=0.8)
            except:
                ax.scatter(rank_single, rank_multi, s=10, c=color, alpha=0.5, rasterized=True)

            # 对角线
            ax.plot([0, 1], [0, 1], 'r-', linewidth=2.5, alpha = 0.8, label = 'y=x')

            # 添加偏离统计
            above_diag = (rank_multi > rank_single).sum()
            below_diag = (rank_multi < rank_single).sum()
            total = len(rank_single)

            ax.text(0.02, 0.98, f'Above diagonal: {above_diag / total:.1%}\n(Multi > Single)',
                    transform=ax.transAxes, fontsize=10, va='top', ha='left',
                    bbox=dict(boxstyle='round', facecolor='white', alpha=0.9))
            ax.text(0.98, 0.02, f'Below diagonal: {below_diag / total:.1%}\n(Multi < Single)',
                    transform=ax.transAxes, fontsize=10, va='bottom', ha='right',
                    bbox=dict(boxstyle='round', facecolor='white', alpha=0.9))

            ax.set_xlabel(f'{mod_name} Rank', fontsize=12, fontweight='medium')
            ax.set_ylabel('Multimodal Rank', fontsize=12, fontweight='medium')
            ax.set_title(f'{mod_name} vs Multimodal\nDensity Distribution',
        ...
        plt.suptitle('Density Distribution:  How Multimodal Changes Rank Relationships',
                     fontsize=14, fontweight='bold', y=1.02)
        plt.tight_layout()
        self._save_figure(fig, output_dir, "6_density_comparison.png")
    ...
    def _plot_modality_complementarity(self, output_dir: str):
        ...
        for col, label, color in zip(cols, labels, colors):
            ...
            y_vals = kde(x_range)
            ax.fill_between(x_range, 0, y_vals, color=color, alpha=0.25)
            ax.plot(x_range, y_vals, color=color, linewidth=3, label=label)

            # 标注中位数
            median_val = np.median(data)
            ax.axvline(x=median_val, color=color, linestyle=':', linewidth=2, alpha=0.7)
            y_at_median = kde(median_val)[0]
            ax.annotate(f'med={median_val:.2f}', xy=(median_val, y_at_median),
                        xytext=(median_val + 0.03, y_at_median + 0.5),
                        fontsize=9, color=color, fontweight='bold',
                        arrowprops=dict(arrowstyle='->', color=color, lw=1.5))

        # 高disagreement阈值线
        ax.axvline(x=0.3, color='#E74C3C', linestyle='--', linewidth=3,
                   label='High disagreement threshold', zorder=5)

        # 高亮高disagreement区域
        ax.axvspan(0.3, 0.8, alpha=0.1, color='#E74C3C')
        ax.text(0.55, ax.get_ylim()[1] * 0.9, 'High\nDisagreement\nZone',
                ha='center', fontsize=11, color='#E74C3C', fontweight='bold', alpha=0.8)
        ...
        ax.set_title('Distribution of Inter-modality Disagreement\n'
                     'Higher disagreement = More complementary information',
                     fontsize=13, fontweight='bold', pad=15)
        ...
        for col, label, color in zip(cols, labels, colors):
            ...
            kde = gaussian_kde(data)
            y_vals = kde(x_range)
            ax.fill_between(x_range, 0, y_vals, color=color, alpha=0.2)
            ax.plot(x_range, y_vals, color=color, linewidth=2.5, label = label)
            ax.axvline(x=0.3, color='#E74C3C', linestyle='--', linewidth=2.5,
            label = 'Threshold (0.3)')
            ax.axvspan(0.3, 0.8, alpha=0.08, color='#E74C3C')
        ...
        def _plot_rank_position_shift(self, output_dir: str):
            ...
            # ===================== Panel A: 条形图（增强版） =====================
            fig_a, ax = plt.subplots(figsize=(10, 7))
            ...
            n_close = [self.rank_shift[k].close_pairs for k in keys]
            ...
            bars2 = ax.bar(x + width / 2, frac_50, width, label='Shifted to >50%',
                           color=colors, alpha=0.45, edgecolor='black', linewidth=1.5, hatch='///')

            # 数值标注
            for bar, val, n in zip(bars1, frac_30, n_close):
            ...
            # 在x轴下方标注close pairs数量
            for i, n in enumerate(n_close):
                ax.text(i, -3, f'n={n}', ha='center', fontsize=10, color='gray')

            ax.set_ylabel('% of Close Pairs Shifted', fontsize=13, fontweight='medium')
            ax.set_title('Non-trivial Resolution:\nClose pairs (Top 5%) shifted to higher ranks in multimodal',
                         fontsize=14, fontweight='bold', pad=15)
            ax.set_xticks(x)
            ax.set_xticklabels(modalities, fontsize=12, fontweight='medium')
            ax.legend(fontsize=11, loc='upper right', framealpha=0.95, edgecolor='gray')
            ax.grid(axis='y', alpha=0.3, linestyle='-', linewidth=0.5)
            ax.set_axisbelow(True)
            ax.set_ylim(0, max(frac_30 + frac_50) * 1.3)
            ...
            # ===================== Panel B: 最佳模态的详细scatter =====================
            best_mod = max(keys, key=lambda k: self.rank_shift[k].fraction_30pct)
            best_idx = keys.index(best_mod)
            color = colors[best_idx]
            mod_name = modalities[best_idx]

            fig_b, ax = plt.subplots(figsize=(9, 8))
            ...
            close_mask = rank_single <= 0.05
            shifted_30 = close_mask & (rank_multi > 0.30)
            shifted_50 = close_mask & (rank_multi > 0.50)

            # 背景
            ax.scatter(rank_single[~close_mask], rank_multi[~close_mask],
                       s=6, c='#CCCCCC', alpha=0.3, rasterized=True, zorder=1)

            # Close但没shift
            close_no_shift = close_mask & ~shifted_30
            ax.scatter(rank_single[close_no_shift], rank_multi[close_no_shift],
                       s=50, c=color, alpha=0.6, edgecolors='white', linewidth=0.8,
                       label=f'Close, no major shift', zorder=2)

            # Shifted到30%以上
            shifted_30_only = shifted_30 & ~shifted_50
            ax.scatter(rank_single[shifted_30_only], rank_multi[shifted_30_only],
                       s=80, c='#F39C12', alpha=0.9, edgecolors='#E74C3C', linewidth=2,
                       label=f'Shifted 30-50% (n={shifted_30_only.sum()})', zorder=3)

            # Shifted到50%以上（最突出）
            ax.scatter(rank_single[shifted_50], rank_multi[shifted_50],
                       s=150, c='#E74C3C', alpha=1.0, edgecolors='black', linewidth=2.5,
            marker = '*', label = f'Shifted >50% (n={shifted_50.sum()})', zorder = 4)

            # 辅助线
            ax.plot([0, 1], [0, 1], 'k-', linewidth=2, alpha=0.7, label='No shift (y=x)')
            ax.axhline(y=0.30, color='#F39C12', linestyle='--', linewidth=2.5, alpha = 0.8)
            ax.axhline(y=0.50, color='#E74C3C', linestyle='--', linewidth=2.5, alpha=0.8)
            ax.axvline(x=0.05, color='gray', linestyle=':', linewidth=2, alpha=0.6)

            # 区域标注
            ax.fill_betweenx([0, 1], 0, 0.05, alpha=0.08, color=color)
            ax.fill_between([0, 0.05], 0.30, 0.50, alpha=0.1, color='#F39C12')
            ax.fill_between([0, 0.05], 0.50, 1.0, alpha=0.15, color='#E74C3C')

            ax.text(0.025, 0.25, 'Preserved', ha='center', va='center', fontsize=10,
                    color='gray', fontweight='bold', alpha=0.7)
            ax.text(0.025, 0.40, 'Moderate\nShift', ha='center', va='center', fontsize=9,
                    color='#F39C12', fontweight='bold', alpha=0.8)
            ax.text(0.025, 0.75, 'Major\nShift', ha='center', va='center', fontsize=10,
                    color='#E74C3C', fontweight='bold', alpha=0.9)

            ax.set_xlabel(f'{mod_name} Distance Rank', fontsize=13, fontweight='medium')
            ax.set_ylabel('Multimodal Distance Rank', fontsize=13, fontweight='medium')
            ax.set_title(f'Rank Shift from {mod_name} Perspective\n'
                         f'{self.rank_shift[best_mod].fraction_30pct:.1%} of close pairs shifted to >30%',
                         fontsize=14, fontweight='bold', pad=15)
            ...
            ax = axes[1]
            ax.scatter(rank_single[~close_mask], rank_multi[~close_mask],
                       s=5, c='#CCCCCC', alpha=0.3, rasterized=True)
            ax.scatter(rank_single[close_no_shift], rank_multi[close_no_shift],
                       s=40, c=color, alpha=0.6, edgecolors='white', linewidth=0.6)
            ax.scatter(rank_single[shifted_30_only], rank_multi[shifted_30_only],
                       s=60, c='#F39C12', alpha=0.9, edgecolors='#E74C3C', linewidth=1.5)
            ax.scatter(rank_single[shifted_50], rank_multi[shifted_50],
                       s=120, c='#E74C3C', alpha=1.0, edgecolors='black', linewidth=2, marker='*')
            ax.plot([0, 1], [0, 1], 'k-', linewidth=1.5, alpha=0.6)
            ax.axhline(y=0.30, color='#F39C12', linestyle='--', linewidth=2, alpha=0.7)
            ax.axhline(y=0.50, color='#E74C3C', linestyle='--', linewidth=2, alpha=0.7)
            ax.axvline(x=0.05, color='gray', linestyle=':', linewidth=1.5, alpha=0.5)
            ax.set_xlabel(f'{mod_name} Distance Rank', fontsize=11)
            ax.set_ylabel('Multimodal Distance Rank', fontsize=11)
            ax.set_title(f'B. Rank Shift from {mod_name}', fontsize=12, fontweight='bold', pad=10)
            ax.set_xlim(-0.02, 1.02)
            ax.set_ylim(-0.02, 1.02)
            ax.set_aspect('equal')
            ax.grid(alpha=0.3)
...
def main():
    ...
    OUTPUT_DIR = "./task2_multimodal_results_v12"
    N_CLUSTERS = 8
    MIN_NEURONS = 10
